...
faithfulness-eval:	## Faithfulness 批量评估（RAGAS LLM Judge）
	@set -a && [ -f .env ] && source .env || true && set +a; \
	PYTHONPATH=src:scripts $(PYTHON_BIN) scripts/daemon/faithfulness_batch.py $(OPTS)
