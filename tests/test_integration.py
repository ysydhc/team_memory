...
CI: set ``TEAM_MEMORY_TEST_DB_URL`` to a dedicated database (e.g.
``postgresql+asyncpg://developer:devpass@localhost:5432/team_memory_test``).
Tests refuse the production DB name ``team_memory`` to avoid accidental data loss.
"""
...
class TestRepositoryIntegration:
    ...
    async def test_feedback(self, session, mock_embed):
        ...
        from team_memory.storage.models import ExperienceFeedback

        res = await session.execute(
            select(func.avg(ExperienceFeedback.rating)).where(
                ExperienceFeedback.experience_id == exp.id
            )
        )
        avg = res.scalar_one()
        assert avg is not None
        assert abs(float(avg) - 3.0) < 0.01
