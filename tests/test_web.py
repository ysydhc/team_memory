...
def setup_app():
    ...
    web_module._settings.architecture = MagicMock(
        provider="gitnexus",
...
class TestCreateExperience:
    ...
    def test_create_experience_with_architecture_nodes(self, client, auth_headers, setup_app):
        """T8: Create experience with architecture_nodes passes to save."""
        with patch("team_memory.web.app.get_session") as mock_gs:
            mock_sess = AsyncMock()
            mock_gs.return_value.__aenter__ = AsyncMock(return_value=mock_sess)
            mock_gs.return_value.__aexit__ = AsyncMock(return_value=False)
            ...
            resp = client.post(
                ...
                json={
                    ...
                    "problem": "Problem",
                    "solution": "Solution",
                    "architecture_nodes": ["src/team_memory/server.py", "src/foo.py"],
                },
            )
            assert resp.status_code == 200
            kwargs = setup_app.save.call_args.kwargs
            assert kwargs.get("architecture_nodes") == ["src/team_memory/server.py", "src/foo.py"]
...
class TestArchitecture:
    ...
    def test_context_returns_200_when_no_bridge(self, client):
        """GET /api/v1/architecture/context returns 200 with available:false when no bridge."""
        resp = client.get("/api/v1/architecture/context")
        assert resp.status_code == 200
        data = resp.json()
        assert data["available"] is False
        assert "reason" in data

    def test_context_returns_real_data_when_bridge_configured(self, client):
        """GET /api/v1/architecture/context returns symbols/relationships when bridge responds."""
        from team_memory.web import app as app_module

        app_module._settings.architecture.gitnexus.bridge_url = (
            "http://127.0.0.1:9321"
        )
        mock_resp = MagicMock()
        ...
        with patch(
            "team_memory.architecture.gitnexus_provider.httpx"
        ) as mock_httpx:
        ...
        assert data["symbols"] == 2066
        assert data["relationships"] == 6085

    def test_graph_returns_empty_when_no_provider(self, client):
        """GET /api/v1/architecture/graph returns 200 with empty graph when provider not configured."""
        with patch(
            "team_memory.web.routes.architecture._get_provider",
            return_value=None,
        ):
            resp = client.get("/api/v1/architecture/graph")
        assert resp.status_code == 200
        data = resp.json()
        assert data.get("nodes") == []
        assert data.get("edges") == []

    def test_impact_requires_path(self, client):
        """GET /api/v1/architecture/impact requires path query param."""
        resp = client.get("/api/v1/architecture/impact")
        assert resp.status_code == 422  # missing required path

    def test_experiences_requires_node(self, client):
        """GET /api/v1/architecture/experiences requires node query param."""
        resp = client.get("/api/v1/architecture/experiences")
        assert resp.status_code == 422  # missing required node

    def test_experiences_returns_200_with_node(self, client):
        """GET /api/v1/architecture/experiences?node=... returns 200 and list."""
        mock_repo = MagicMock()
        mock_repo.list_experiences_by_node = AsyncMock(return_value=[])
        ...
        with (
            ...
            patch(
                "team_memory.web.routes.architecture.get_session",
                return_value=mock_session_ctx,
            ),
            patch(
                "team_memory.web.routes.architecture.ExperienceRepository",
                return_value=mock_repo,
            ),
        ):
            resp = client.get(
                "/api/v1/architecture/experiences?node=src/foo.py"
            )
        assert resp.status_code == 200
        assert resp.json() == []

    def test_search_requires_q(self, client):
        """GET /api/v1/architecture/search returns 400 when q is missing or empty."""
        resp = client.get("/api/v1/architecture/search")
        assert resp.status_code == 400
        resp2 = client.get("/api/v1/architecture/search?q=")
        assert resp2.status_code == 400

    def test_search_rejects_invalid_q(self, client):
        """GET /api/v1/architecture/search returns 400 when q contains invalid chars."""
        resp = client.get("/api/v1/architecture/search?q=foo%3Bbar")
        assert resp.status_code == 400
        assert "invalid" in resp.json().get("detail", "").lower()

    def test_search_returns_empty_when_no_provider(self, client):
        """GET /api/v1/architecture/search returns 200 with empty nodes when provider not configured."""
        with patch(
            "team_memory.web.routes.architecture._get_provider",
            return_value=None,
        ):
            resp = client.get("/api/v1/architecture/search?q=auth")
        assert resp.status_code == 200
        assert resp.json().get("nodes") == []

    def test_search_returns_empty_when_bridge_unavailable(self, client):
        """GET /api/v1/architecture/search returns 200 with empty nodes when Bridge unreachable."""
        from team_memory.architecture.gitnexus_provider import BridgeUnavailableError

        mock_provider = MagicMock()
        mock_provider.search_nodes = AsyncMock(side_effect=BridgeUnavailableError("Connection refused"))
        with patch(
            "team_memory.web.routes.architecture._get_provider",
    ...
    def test_search_returns_nodes_when_bridge_configured(self, client):
        """GET /api/v1/architecture/search returns nodes when bridge responds."""
        mock_provider = MagicMock()
        mock_provider.search_nodes = AsyncMock(
            return_value=[
                {"id": "File:src/auth.py", "label": "auth", "path": "src/auth.py", "kind": "File"},
            ]
        )
        with patch(
            "team_memory.web.routes.architecture._get_provider",
        ...
        assert data["nodes"][0]["path"] == "src/auth.py"

    def test_experiences_returns_list_when_bound(self, client):
        """GET /api/v1/architecture/experiences returns list when bindings exist."""
        mock_repo = MagicMock()
        mock_repo.list_experiences_by_node = AsyncMock(
            return_value=[
                {
                    "experience_id": "a1b2c3d4-0000-0000-0000-000000000001",
                    "title": "Fix server startup",
                    "node": "src/team_memory/server.py",
                },
            ]
        )
        mock_session = MagicMock()
        ...
        with (
            ...
            resp = client.get(
                "/api/v1/architecture/experiences?node=src/team_memory/server.py"
            )
        assert resp.status_code == 200
        data = resp.json()
        assert len(data) == 1
        assert data[0]["experience_id"] == "a1b2c3d4-0000-0000-0000-000000000001"
        assert data[0]["title"] == "Fix server startup"
        assert data[0]["node"] == "src/team_memory/server.py"
