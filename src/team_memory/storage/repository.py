...
from sqlalchemy import Float, delete, desc, func, select, text, update
...
class ExperienceRepository:
    ...
    @staticmethod
    def _active_filter():
        """Standard filter: only non-deleted, published experiences."""
        return [
    ...
    async def count_by_tags(self, tags: list[str], project: str | None = None) -> int:
    ...
    async def list_pending_review(self, limit: int = 50) -> list[Experience]:
        """List experiences pending review."""
        result = await self._session.execute(
            select(Experience)
            .where(Experience.is_deleted == False)  # noqa: E712
            .where(Experience.review_status == "pending")
            .order_by(desc(Experience.created_at))
            .limit(limit)
        )
        return list(result.scalars().all())
    ...
    async def count(
        self, include_deleted: bool = False, project: str | None = None
    ) -> int:
        """Count total number of experiences."""
        query = (
            select(func.count())
            .select_from(Experience)
            .where(Experience.project == self._project_value(project))
    ...
    async def search_by_vector_grouped(
        ...
        """
        # Search across all experiences (parents + children)
    ...
    async def list_drafts(
        ...
        query = (
            select(Experience)
            .where(Experience.is_deleted == False)  # noqa: E712
            .where(Experience.publish_status == "draft")
            .where(Experience.parent_id.is_(None))
            .where(Experience.project == self._project_value(project))
            .options(selectinload(Experience.children))
            .order_by(desc(Experience.created_at))
    ...
    async def get_experiences_without_summary(
        self, limit: int = 10, min_content_length: int = 500
    ) -> list[Experience]:
    ...
    async def count_stale(self, months: int = 6) -> int:
    ...
    async def get_stats(self) -> dict:
        ...
        tag_query = text("""
            ...
            WHERE is_deleted = false AND publish_status = 'published'
            GROUP BY tag
            ORDER BY cnt DESC
            LIMIT 20
        """)
        tag_result = await self._session.execute(tag_query)
        ...
        recent_query = text("""
            ...
            FROM experiences
            WHERE created_at > NOW() - INTERVAL '7 days'
              AND is_deleted = false AND publish_status = 'published'
        """)
        recent_result = await self._session.execute(recent_query)
        ...
        pending_query = text("""
            SELECT COUNT(*)
            FROM experiences
            WHERE review_status = 'pending' AND is_deleted = false
        """)
        pending_result = await self._session.execute(pending_query)
        pending_count = pending_result.scalar_one()

        # Stale experiences (last_used_at > 6 months ago)
        stale_query = text("""
            SELECT COUNT(*)
            FROM experiences
