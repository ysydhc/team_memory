...
> 本文档为详细版。快速上手见 [README](../README.md)。
...
> 这是我在学 AI 时萌生的一个想法。市面上已有类似产品，但总觉得不太贴合自己的使用习惯。做这个项目，既想通过和 AI 一起写代码来加深对大模型的理解，也希望能按自己的工作流，打磨出真正顺手的功能。

基于 MCP 协议的团队经验数据库，解决 AI 编程助手「每次对话都从零开始」的核心痛点。
...
**扩展功能**（任务管理、Skills、工作流）见 [docs/EXTENDED.md](docs/EXTENDED.md)。
...
| 盲区 | 现象 | 影响 |
|------|------|------|
| **无记忆** | 上周刚帮你修过的 Bug，这周遇到类似的，它完全不记得 | 重复排查，浪费时间和 Token |
| **只见代码，不懂决策** | 能看懂「是什么」，却不知道「为什么这么写」「上次踩过什么坑」 | 可能重蹈覆辙，或推翻合理的历史决策 |
| **静态知识不够用** | Rules、Skills 管得了规范，管不住每天冒出来的隐性经验（接口坑、故障根因、被否掉的方案） | AI 缺乏团队的「隐性知识」，始终像个新人 |

**TeamMemory 就是冲着这三个问题来的。** 通过 MCP 把语义可搜索的经验库接进 AI，让 AI 能够：
- 遇到问题时，自动查询团队是否曾经解决过类似问题
- 解决问题后，自动从对话中提取关键经验并存储
- 下次任何团队成员遇到同类问题，AI 直接命中历史方案

既适合 3–10 人的技术团队共享，也适合部署在本地个人使用，配合 Cursor / Claude Desktop。
...
| **MCP** | Model Context Protocol，让 AI 客户端调用 tm_search、tm_save 等工具 |
...
- **经验组**（`tm_save_group`）：将一次完整的问题解决过程（从发现到排查到修复）作为父子经验组保存；三阶段管道：提取→结构化→保存
...
- **核心 MCP 工具**：`tm_solve`、`tm_search`、`tm_suggest`、`tm_learn`、`tm_save`、`tm_feedback` 等，覆盖搜索、保存、反馈全场景
- **3 个 MCP 资源**：最近经验、统计数据、过期经验
- **7 个 MCP Prompt**：对话摘要、文档提交、质量审核、故障排查、长文档分析、任务执行、项目恢复
- **Web 管理界面**：浏览、搜索、审核、配置一站式管理

任务、Skills、工作流等扩展工具见 [docs/EXTENDED.md](docs/EXTENDED.md)。
...
| `tm_doc_sync` | 文档同步 | file_path + content + doc_type + project | 将本地文档幂等同步到经验库 |
...
任务 Kanban、归档管理、使用统计等扩展页面见 [docs/EXTENDED.md](docs/EXTENDED.md)。
...
| `make hooks-install` | 安装 Git hooks（任务关联） | 复制 `scripts/post-commit-hook.sh` 到 `.git/hooks/post-commit`；任务扩展见 [EXTENDED](docs/EXTENDED.md) |
...
- **功能或代码变更后**，须同步更新 **README**；文档不同步不得视为任务完成。
