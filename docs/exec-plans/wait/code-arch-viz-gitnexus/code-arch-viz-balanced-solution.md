...
- **技术最简化**：采纳 Python + ast、PostgreSQL 双表、仅 IMPORTS、文件级 MVP、7 步实现、经验挂载约定与只读 API；不采纳图库、执行流、细粒度全量图谱进 MVP。
- **项目收益最大化**：采纳 P0/P1/P2 优先级（关联模型、按位置检索、任务完成写节点、影响面、手动挂载）；不采纳 P3/P4 进 MVP（任务–架构展示、模块聚类、执行流深度结合留 Phase 2+）。
- **使用最易**：采纳零配置出图、≤3 步路径、三句话术、Web 架构页 + 节点侧栏 + 经验挂载/展示、MCP 只读与挂载、错误降级；不采纳「执行流/影响面一键」进 MVP（放 Phase 2），推荐挂载可简化实现。
