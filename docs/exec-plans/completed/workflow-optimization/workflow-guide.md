...
本文用于指导工作流搭建。执行与恢复以 **TM 任务状态 + tm_message** 为准，不使用状态文件。流程定义与运行状态分离，单源为 `.tm_cursor/plans/workflows/*.yaml`。
...
- **位置**：`.tm_cursor/plans/workflows/<工作流名称>.yaml`
...
- **目录**：流程定义仅放在 `.tm_cursor/plans/workflows/` 下，无状态文件。
...
|------|------|
| 本文 | 工作流搭建指导 |
| .tm_cursor/plans/workflows/task-execution-workflow.yaml | 可编辑任务执行工作流（YAML 单源，含 step-coldstart 冷启动） |
