...
> **状态**：草案 → 执行跟踪：`docs/exec-plans/executing/personal-memory-profile-alignment/execute.md` · **任务清单（逐条验收）**： [tasks.md](../exec-plans/executing/personal-memory-profile-alignment/tasks.md)  
> **参考**： [supermemory-comparison.md](../design-docs/supermemory-comparison.md)（§4.2 User Profiles）  
> **MCP 默认入口**：[mcp-lite-default.md](../design-docs/ops/mcp-lite-default.md)（`make mcp` = Lite）  
> **日期**：2026-04-01  
...
**文档债务**：`mcp-patterns.md`、Cursor 规则、Lite Instructions **只描述四个 `memory_*`**；删除或替换仍指向 **`tm_preflight` / `tm_suggest` / `tm_search`** 的幽灵表述。  

### 3.5 观测与文档同步

- 日志：`personal_memory: saved ...`（可含 static/dynamic 计数；E 后可 debug 统计含 `valid_until` 的写入）。  
- 更新：troubleshooting §12、supermemory 对比文档（画像 + E 后「自动遗忘」）、**`mcp-patterns.md` 的 Lite 工具与返回体**（不登记新工具名）。  
...
| **D** | ~0.5 周 | C | **`server_lite` Instructions** + rules + **`mcp-patterns` Lite**；无幽灵 `tm_*` / `tm_preflight` |
...
3. **`memory_recall(include_user_profile)`**：按 §3.4 定 **P0 或 P1**，并更新 **`mcp-patterns.md`（Lite）**。  
...
对外一句：**错误画像可通过 Web/HTTP 删除或管理员协助**（细节见 `getting-started` / 安全页）。  
